...
from user.schema import (
    UserSignUpRequest, 
    UserLogInRequest, 
    UserHealthProfileCreateRequest, 
    UserResponse, 
...
async def create_health_profile_handler(
    ...
    if existing_profile:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="이미 생성된 건강 프로필이 존재합니다."
        )

    # 새로운 건강 프로필 생성
    profile_data: dict = body.model_dump()
    new_profile = UserHealthProfile(user_id=user_id, **profile_data)

    session.add(new_profile)
    await session.commit()
    await session.refresh(new_profile)  # id, created_at 새로고침
